...
sys.path.append("/gpfs/data/huo-lab/Image/annawoodard/maicara/maicara")
...
parser.add_argument(
    "pretrained", type=Path, metavar="FILE", help="path to pretrained model"
)
parser.add_argument(
    "--weights",
    default="freeze",
...
parser.add_argument(
    ...
    type=int,
    metavar="N",
    help="max classes to load from image dir",
)
...
class ImageFolder(datasets.DatasetFolder):
    ...
    def __init__(
        ...
        loader: Callable[[str], Any] = None,  # datasets.default_loader,
        is_valid_file: Optional[Callable[[str], bool]] = None,
    ):
        super(ImageFolder, self).__init__(
            root,
            loader,
            datasets.IMG_EXTENSIONS if is_valid_file is None else None,
            transform=transform,
...
def main_worker(gpu, args):
    ...
    if args.rank == 0:
        ...
        run = wandb.init(config=args, project="barlowtwins")
    ...
    train_dataset = datasets.ChiMECSSLDataset(
        traindir,
        transforms.Compose(
            [
    ...
    val_dataset = datasets.ChiMECSSLDataset(
        valdir,
        transforms.Compose(
            [
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (images, target) in enumerate(
            ...
            print("targets less than zero ", target[target < 0])
            print("target ", target)
        ...
        if args.weights == "freeze":
            ...
            for k in reference_state_dict:
                assert torch.equal(
                    model_state_dict[k].cpu(), reference_state_dict[k]
                ), k
